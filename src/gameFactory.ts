...
export class GameFactory extends Base {
  ...
  async getAllGames(address?: string, round?: number) {
    ...
    for (const game of resp.applications) {
      ...
      let activeGames: ActiveGameState[] = []
      let newgame: GameState = {name: name, url: url, appID: game.id, game_type: Number(gtype), embed: (embed>0), activeGames: activeGames}
      ...
      for (const activeGame of resp.applications) {
        ...
        let newActiveGame: ActiveGameState = {name: name, url: url, appID: activeGame.id, game_type: Number(gtype), embed: (embed>0), cost: Number(cost), arcade: Number(arcade), ticket: Number(ticket), game: game.id, arcadeOwner: ""}
        if (!!address) {
          if (arcade > 0) {
  ...
  async getGame(activeGameAppID: number, address: string, round?: number) {
    if (!!!address) {
      return null
    }
